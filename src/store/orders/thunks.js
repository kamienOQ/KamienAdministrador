  ...
  return async (dispatch, getState) => {
    ...
    if(field?.includes('name')){
      if(value !=='asc' && value !== 'desc'){
        ...
        if(preValue !== value){
          q = query( collectionRef, where('nameLowerCase', '>=', formattedName), where('nameLowerCase', '<', formattedName + '\uf8ff'));
          const querySnapshot = await getDocs(q);
          undersized = (querySnapshot.size <= size) ? true : false;
          dispatch(onSetNumberOrders(querySnapshot.size));
        } if (page === 0 || undersized) {
          page = 0;
          q = query( collectionRef, where('nameLowerCase', '>=', formattedName), where('nameLowerCase', '<', formattedName + '\uf8ff'), limit(size) );
        } else {
          const lastVisibleDoc = query( collectionRef,  where('nameLowerCase', '>=', formattedName), where('nameLowerCase', '<', formattedName + '\uf8ff'), limit(page * size) );
          const lastVisibleDocSnapshot = await getDocs(lastVisibleDoc);
          const lastVisible = lastVisibleDocSnapshot.docs[lastVisibleDocSnapshot.docs.length-1];
          q = query( collectionRef,  where('nameLowerCase', '>=', formattedName), where('nameLowerCase', '<', formattedName + '\uf8ff'), startAfter(lastVisible), limit(size) );
        }
      } else {
        if (page === 0) {
