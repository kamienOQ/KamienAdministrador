...
export const productsSlice = createSlice({
  ...
  reducers: {
    ...
    onChargeProductsUploaded: (state, { payload }) => {
      ...
      if (state.products) {
        state.products.forEach((product) => {
          if (product.productName === payload.productName) duplicate = true;
        });
        if (!duplicate) {
    ...
    onAddPhotoProduct: (state, { payload }) => {
      state.activeProduct.photo.name = payload[0];
      state.activeProduct.photo.url = payload[1];
    },
    onChargeProductsByPage: (state, { payload }) => {
      state.productsOnPage = payload;
    },
    onAddLowerCase: (state) => {
      state.activeProduct.productNameLowerCase =
        state.activeProduct.productName.toLowerCase();
    },
    ...
    onAddProductNameLowerCase: (state) => {
      ...
      state.activeProduct.productNameLowerCase = formattedName
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "");
    },
